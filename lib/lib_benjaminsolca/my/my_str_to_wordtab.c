...
** Last update Thu Apr  6 08:34:53 2017 Benjamin
*/
...
static int	count_string(char *str, char *parse, int i)
{
  int		mem;
...
char	**my_str_to_wordtab(char *str, char *parse)
{
  char	**tab;
  int	ijkl[4];
  ...
  if (str == NULL)
    return (NULL);
  if (!(tab = malloc(sizeof(char *) * (count_wordtab(str, parse) + 1))))
    return (NULL);
  while (ijkl[3]++ < count_wordtab(str, parse))
	...
	ijkl[0]++;
      ...
      if (str[ijkl[0]] != '\0')
	{
	  if (!(tab[ijkl[1]] = malloc(sizeof(char)
				      * (count_string(str, parse,
						      ijkl[0]) + 1))));
